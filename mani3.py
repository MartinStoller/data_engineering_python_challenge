...
def process_file(file_id: int, chunk_size: int, products: pd.DataFrame, eur_usd_conversion: pd.DataFrame) -> pd.DataFrame:
    ...
    with pd.read_csv('transactions/' + str(file_id) + '.csv', chunksize=chunk_size) as reader:
        ...
        with mp.get_context(method='spawn').Pool() as pool:
            for transactions, intermediate_result in pool.imap_unordered(partial(process_chunk, eur_usd_conversion=eur_usd_conversion, file_id=file_id, products=products), reader):
                output_path = Path('./sales_postprocessed_' + str(file_id) + '.parquet')
                if output_path.exists():
                    transactions.to_parquet('sales_postprocessed_' + str(file_id) + '.parquet', engine='fastparquet', append=True)
                else:
                    transactions.to_parquet('sales_postprocessed_' + str(file_id) + '.parquet', engine='fastparquet')
                if result_df.empty:
...
def process_chunk(chunk, eur_usd_conversion: pd.DataFrame, file_id: int, products: pd.DataFrame) -> pd.DataFrame:
    ...
    return transactions, transactions[['product_id', 'amount', 'production_costs', 'net_profit']].groupby('product_id').sum()
...
if __name__ == '__main__':
    ...
    for file_id in file_ids:
        intermediate_result = process_file(file_id, int(5e5), products, eur_usd_conversion)
        if result.empty:
